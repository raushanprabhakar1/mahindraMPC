...
int main(int argc, char** argv)
{
	...
	while(ros::ok())
    {
		...
		cte_double_dot1 += (((((-Cf-Cr)*cte_dot1)/(M*v))+(((Cf+Cr)*psi_dot1))/M)+(((lr*Cr-lf*Cf)*psi_double_dot1)/(M*v)+(Cf*M/delta0))*dt);
		//cte_double_dot1=1;
		...
		cout<<" v  "<<v<<endl;
		//cout<<"y1  "<<y1<<endl<<" psi_double_dot1  "<<psi_double_dot1<<endl<<"psi1  "<<endl<<psi1<<"v1  "<<v1<<endl<<"cte1  "<<cte1<<endl;
